...
# read data 
data = pd.read_csv('./niftydata/ALL.RELIANCE.NS.csv')
...
money =0
repitition = 1
end_price = scl[0].inverse_transform(y_test[0][-1])[0][0]
start_price = scl[0].inverse_transform(X_train[0][-1,0].reshape(-1,1))[0][0]

for i in range(0,X_main_test.shape[0],repitition):
	temp = X_main_test[i]
	prediction = []
	...
	predicted_price = Y[-1]
	today_price = scl[0].inverse_transform(X_test[0][i,-1].reshape(-1,1))[0][0]
	...
	X_main_train = np.append(X_main_train,X_main_test[i:i+repitition],axis=0)
	y_main_train = np.append(y_main_train,y_main_test[i:i+repitition],axis=0)
	for k in range(0,features):
		X_train[k] = np.append(X_train[k],X_test[k][i:i+repitition],axis=0)
		y_train[k] = np.append(y_train[k],y_test[k][i:i+repitition],axis=0)

	# trainModel(X_train[-100:],y_train[-100:],model)
	if((i)%(50) == 49):
		trainModel(X_main_train,y_main_train,model[0])
		for k in range(1,features):
			trainModel(X_train[k],y_train[k],model[k])


print start_price, end_price
print 'Profit - ',
print ((stocks*end_price + money)/start_price - 1)*100 , '%'
plt.show()
