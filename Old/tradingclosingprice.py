...
data = pd.read_csv('./niftydata/Infosys_news.csv')
...
stocks=1 
money =0
end_price = scl.inverse_transform(y_test[-1].reshape(-1,1))[0][0]
start_price = scl.inverse_transform(X_train[-1,-1].reshape(-1,1))[0][0]
...
for i in range(0,X_test.shape[0],repitition):
	...
	predicted_price = Y[-1][0]
	today_price = scl.inverse_transform(X_test[i][-1].reshape(-1,1))[0][0] 
	print stocks, money, predicted_price, today_price, predicted_price > today_price
...
print 'Profit - ',
print (stocks*end_price + money)/start_price , '%'
